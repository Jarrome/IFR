...
class ThreeDMatch_Testing(torch.utils.data.Dataset):
    def __init__(self, dataset_path, category, overlap_ratio, voxel_ratio, voxel, max_voxel_points, num_voxels, rigid_transform, vis,data_type='synthetic'):
    ...
    def __getitem__(self, index):
        ...
        if self.data_type == 'synthetic':
            ...
            #p0_pre -= p0_pre.mean(axis=0,keepdims=True)
            #p1_pre -= p1_pre.mean(axis=0,keepdims=True)
         
            x = torch.from_numpy(self.perturbation[index][np.newaxis,...])
            p1, igt = self.do_transform(torch.from_numpy(p1_pre), x)
            p0 = torch.from_numpy(p0_pre)
            return p0, p1, igt.float()
        else:
            ...
            if len(idx_conditioned) == 0:
                idx_conditioned_p0 = coords_p0_idx[np.where(num_points_per_voxel_p0>=0.01*self.max_voxel_points)]
                idx_conditioned_p1 = coords_p1_idx[np.where(num_points_per_voxel_p1>=0.01*self.max_voxel_points)]
                idx_conditioned, _, _ = np.intersect1d(idx_conditioned_p0, idx_conditioned_p1, assume_unique=True, return_indices=True)
...
class PointRegistration_fixed_perturbation(torch.utils.data.Dataset):
    def __init__(self, dataset, rigid_transform, sigma=0.00, clip=0.00, point_percent=1.):
    ...
    def __getitem__(self, index):
        pm, _ = self.dataset[index]   # one point cloud
        '''
        if pm.shape[0] > 200000:
            # too many points in synthetic data...
             perm = torch.randperm(pm.size(0))
             idx = perm[:200000]
             pm = pm[idx]
        '''
        ...
        p_ = add_noise(pm, sigma=self.sigma, clip=self.clip)
        # sparsity
        if self.point_percent != 1.:
            perm = torch.randperm(p_.size(0))
            if int(p_.size(0)*self.point_percent) > 1:
                idx = perm[:int(p_.size(0)*self.point_percent)]
            else:
                idx = perm[:2]
            p_ = p_[idx]
        ...
        #np.save("noise_data_example/"+str(index)+'_2.npy', p_)
...
def glob_dataset(root, class_to_idx, ptns):
    ...
    for target in tqdm.tqdm(sorted(os.listdir(root))):
...
class Stanford():
    def __init__(self, dataset_path):
        filenames = glob.glob(os.path.join(dataset_path,'*.pcd'))
        self.datas = [np.array(o3d.io.read_point_cloud(filename).points) for filename in filenames]
        self.datas = [torch.from_numpy(data).type(dtype=torch.float) for data in self.datas]
        self.transform = OnUnitCube() 
    def __getitem__(self, idx):
        return self.transform(self.datas[idx]), None
