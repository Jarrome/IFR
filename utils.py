...
def feature_jac(M, A, Ax, BN, device, max_idx=None):
    # M, A, Ax, BN: list
    A1, A2, A3 = A
    M1, M2, M3 = M
    Ax1, Ax2, Ax3 = Ax
    ...
    if max_idx is None:
        ...
        # c_out x 1 x B x N
        assert(max_idx.shape[0]==1)

        M1 = M1.detach().unsqueeze(1).transpose(0,2)
        M2 = M2.detach().unsqueeze(1).transpose(0,2)
        M3 = M3.detach().unsqueeze(1).transpose(0,2)
        dBN1 = dBN1.transpose(0,2)
        dBN2 = dBN2.transpose(0,2)
        dBN3 = dBN3.transpose(0,2)
        ...
        A1BN1M1_A2BN2M2 = torch.einsum('ijkl,ikml->ijml', A1BN1M1, A2BN2M2)   # B x 3 x 64 x N_related
        A2BN2M2_A3BN3M3 = torch.einsum('ijkl,ikml->ijml', A1BN1M1_A2BN2M2, A3BN3M3)   # B x 3 x K x N_related
